...
def process_video(video_path, max_frames=64):
    ...
    return base64Frames, frame_time_str, video_time
...
def analyze_video(video_path, json_item, api_key, max_frames=64):
    ...
    frames, frame_time, video_time = process_video(video_path, max_frames)
    ...
    prompt_content = (
        ...
        f"{options_text}\n\n"
        f"Based on what you observe in the video frames, which option provides the most accurate feedback? "
...
def process_videos(video_dir, json_file, api_key, max_frames=64):
    ...
    json_data = load_json_or_jsonl(json_file)
    ...
    output_file = os.path.join(os.path.dirname(video_dir), "gpt4o_analysis_results_final.json")
...
def main():
    parser = argparse.ArgumentParser(description="Analyze options in videos using GPT-4o model")
