...
def load_json_or_jsonl(path):
    """Load evaluation metadata from either a .json (list) or .jsonl (one JSON object per line).
    ...
    if path.endswith('.jsonl'):
        ...
        with open(path, 'r') as f:
            ...
            if isinstance(loaded, dict):
                # Normalize to list if a single object is provided
                return [loaded]
            return loaded
...
def main():
    ...
    parser.add_argument("--json", type=str, default="exact.json",
                       help="Path to the JSON or JSONL metadata file with options")
